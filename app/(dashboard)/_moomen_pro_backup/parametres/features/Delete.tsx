...
export function Delete({ isOpen, onClose, onSuccess, data }: DeleteProps) {
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await apiFetch(`/features/delete/${data.id}`, {
                method: "DELETE",
            });
                    ...
                    <div className="flex flex-col items-center text-center space-y-4">
                        ...
                        <div className="space-y-2">
                            <p className="text-slate-600">
                                ...
                                <span className="font-bold text-red-600">{data.libelle}</span> ({data.code}) ?
                            </p>
                            <p className="text-sm text-slate-400 italic">
                                Cette action pourrait affecter les permissions des rôles utilisant cette fonctionnalité.
                            </p>
                        </div>
                        ...
                        <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100 w-full">
                            ...
                            <div className="text-left">
                                <p className="text-xs text-slate-400">Code Système</p>
                                <p className="font-bold text-slate-700 font-mono">{data.code}</p>
                            </div>
                        </div>
                    </div>
                </div>
