...
export function Add({ isOpen, onClose, onSuccess, size = "md" }: Props) {
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        try {
            await apiFetch("/civilite/create", {
                method: "POST",
                data: form
            });
            toast.success("Civilité créée !");
        ...
        >
            <form onSubmit={handleSubmit} className="space-y-4 py-2">
                <div className="space-y-2">
                    ...
                    <Input value={form.code} onChange={set("code")} placeholder="Ex: M., Mme" required />
                </div>
                <div className="space-y-2">
                    <Label>Libellé <span className="text-red-500">*</span></Label>
                    <Input value={form.libelle} onChange={set("libelle")} placeholder="Ex: Monsieur, Madame" required />
                </div>
            </form>
        </Modal>
    );
