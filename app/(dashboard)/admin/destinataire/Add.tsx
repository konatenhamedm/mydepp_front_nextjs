...
export function Add({ isOpen, onClose, onSuccess, size = "md" }: Props) {
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        try {
            await apiFetch("/destinateur/create", {
                method: "POST",
                data: form
            });
        ...
        <Modal
            ...
            title={
                <div className="flex items-center gap-3">
                    <div className="bg-[#EBF2FF] p-2 rounded-lg text-[#0052CC]"><Send className="h-5 w-5" /></div>
        ...
        >
            <form onSubmit={handleSubmit} className="space-y-4 py-2">
                <div className="space-y-2">
                    ...
                    <Input value={form.code} onChange={set("code")} placeholder="Ex: DIR" required />
                </div>
                <div className="space-y-2">
                    <Label>Libellé <span className="text-red-500">*</span></Label>
                    <Input value={form.libelle} onChange={set("libelle")} placeholder="Ex: Direction Générale" required />
                </div>
            </form>
        </Modal>
    );
