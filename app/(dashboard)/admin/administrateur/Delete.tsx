...
export function Delete({ isOpen, onClose, onSuccess, data, size = "md" }: Props) {
    ...
    const handleDelete = async () => {
        ...
        try {
            await apiFetch(`/user/admin/delete/${data.id}`, {
                method: "DELETE"
            });
            ...
            <div className="py-4 text-center space-y-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                        Êtes-vous sûr de vouloir supprimer l'administrateur <span className="font-bold text-slate-700">"{data.personne?.nom} {data.personne?.prenoms}"</span> ?
                        L'accès de cet utilisateur sera définitivement révoqué.
                    </p>
                </div>
            </div>
        </Modal>
