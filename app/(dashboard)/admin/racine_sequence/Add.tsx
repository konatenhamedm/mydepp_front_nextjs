...
export function Add({ isOpen, onClose, onSuccess, size = "md" }: Props) {
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        try {
            await apiFetch("/racineSequence/create", {
                method: "POST",
                data: form
            });
            toast.success("Racine créée !");
            setForm({ libelle: "" });
            onSuccess();
            onClose();
        } catch (err: any) {
        ...
        <Modal
            ...
            title={
                <div className="flex items-center gap-3">
                    <div className="bg-[#EBF2FF] p-2 rounded-lg text-[#0052CC]"><Hash className="h-5 w-5" /></div>
                    Nouvelle racine séquence
                </div>
            }
            size={size}
            footer={
