...
export default function DistrictPage() {
    ...
    const filteredData = Array.isArray(data)
        ? data.filter((item) =>
            ...
            item.region_libelle?.toLowerCase().includes(searchTerm.toLowerCase())
        )
        : [];
    ...
    return (
        <ClientOnly>
            <div className="space-y-5">
                <PageHeader
                    title="Districts"
                    description="Gestion des districts sanitaires/administratifs"
                ...
                <SearchBar
                    ...
                    placeholder="Rechercher par libellé ou région..."
                    onRefresh={refreshData}
                    isLoading={isLoading}
                />
                <DataTable
                    title="Liste des districts"
                    titleIcon={<Navigation className="w-4 h-4" />}
                    footer={
                        <Pagination
                    ...
                    <table className="w-full">
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                <EmptyState message="Aucun district trouvé" icon={<Navigation className="w-10 h-10" />} cols={COLS} />
                            ) : (
                                currentitems.map((item, index) => (
                                    <tr key={item.id ?? index} className="hover:bg-slate-50 transition-colors duration-100 group">
                                        ...
                                        <td className={TD.muted}>{item.region_libelle || item.region?.libelle || "N/A"}</td>
