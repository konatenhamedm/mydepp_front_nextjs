...
export function Edite({ isOpen, onClose, onSuccess, data, size = "md" }: Props) {
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        try {
            await apiFetch(`/direction/update/${data.id}`, {
                method: "PUT",
                data: form
            });
        ...
        >
            <form onSubmit={handleSubmit} className="space-y-4 py-2">
                <div className="space-y-2">
                    ...
                    <Input value={form.libelle} onChange={e => setForm(p => ({ ...p, libelle: e.target.value }))} placeholder="Ex: Direction des Etudes" required />
                </div>
            </form>
        </Modal>
    );
