...
import { Plus, Users, Phone, Mail, Store, MapPin, Search, Edit3, Trash2, Eye } from "lucide-react";
      ...
      <ClientOnly>
        <div className="flex flex-col items-center justify-center py-20 text-center gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-slate-400 text-sm font-medium">Veuillez choisir un établissement pour voir vos clients</p>
          </div>
        </div>
      </ClientOnly>
    );
    ...
    <ClientOnly>
      <div className="space-y-6">
        <PageHeader
          ...
          description={`Portefeuille client de ${magasin?.libelle || "votre établissement"}`}
        ...
        <SearchBar
          ...
          placeholder="Filtrer par nom, téléphone, adresse..."
        ...
        >
          <table className="w-full">
            <TableHeaderCustom
              items={["Client & Contact", "Coordonnées", "Adresse Géographique"]}
              afficheAction={true}
              actionWidth="160px"
            />
            <tbody>
              {isLoading ? (
                ...
                <EmptyState message="Aucun client enregistré" icon={<Users className="w-10 h-10" />} cols={COLS} />
              ) : (
                  ...
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-all duration-200 group border-b border-slate-50 last:border-0">
                    <td className={TD.base}>
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-2xl bg-[#0052cc]/5 flex items-center justify-center text-[#0052cc] font-black text-xs shadow-sm group-hover:bg-[#0052cc] group-hover:text-white transition-all">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-1">
                            <Phone className="w-2.5 h-2.5" /> {item.tel || "N/A"}
                          </span>
                        </div>
                      </div>
                    </td>
                    <td className={TD.base}>
                      <div className="flex flex-col gap-1">
                        ...
                        ) : <span className="text-[10px] text-slate-300 italic">Pas d'email</span>}
                      </div>
                    </td>
                    <td className={TD.muted}>
                      <div className="flex items-center gap-2 max-w-[250px]">
                        <MapPin className="w-3 h-3 text-slate-300 shrink-0" />
                        <span className="truncate text-[11px] font-black uppercase tracking-wider">{item.adresse || "Localisation non définie"}</span>
                      </div>
                    </td>
