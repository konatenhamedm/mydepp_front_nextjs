...
export function Delete({ isOpen, onClose, onSuccess, data, size = "md" }: Props) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            await apiFetch("/categorie_produit_services/delete", {
                method: "POST",
                data: { ids: [data.id] }
            });
    ...
    return (
        ...
        >
            <div className="py-2">
                <p className="text-slate-600">Voulez-vous supprimer la catégorie <span className="font-bold text-red-600">{data?.libelle}</span> ?</p>
