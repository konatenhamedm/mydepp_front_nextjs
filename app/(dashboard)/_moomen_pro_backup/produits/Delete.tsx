...
export function Delete({ isOpen, onClose, onSuccess, data, size = "md" }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await apiFetch(`/produits/${data.id}/delete`, { method: "DELETE" });
      toast.success("Produit supprimé !"); onSuccess(); onClose();
    ...
    >
      <div className="py-2">
        <p className="text-slate-600">
          Voulez-vous supprimer le produit <span className="font-bold text-red-600">{data?.libelle ?? data?.id}</span> ?
        </p>
