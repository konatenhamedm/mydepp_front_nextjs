...
import { Plus, Ruler, Store } from "lucide-react";
...
export default function Page() {
    ...
    const refreshData = () => {
        if (!magasinId) return;
        setIsLoading(true);
        apiFetch(`/unites/all/magasin/{id}`.replace("{id}", String(magasinId)))
    ...
    const filteredData = Array.isArray(data)
        ? data.filter((item) => searchTerm === "" || item.libelle?.toLowerCase().includes(searchTerm.toLowerCase()) || item.abr?.toLowerCase().includes(searchTerm.toLowerCase()))
        : [];
            ...
            <div className="space-y-5">
                ...
                >
                    <table className="w-full">
                        <TableHeaderCustom items={["#", "Libellé", "Abréviation", "Décimales"]} afficheAction={true} actionWidth="100px" />
                        <tbody>
                            {isLoading ? (
