...
export default function AdminDashboard() {
    ...
    const fetchData = async () => {
        ...
        try {
            if (isAdminGeneral) {
                const res = await apiFetch("/statistique/admin/general", { method: "GET" });
                setDataRaw(res.data);
                ...
                {dataView ? (
                    <>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                            {isAdminGeneral ? (
                                <>
                                    <KpiCard title="Total Utilisateurs" value={dataView.utilisateurs?.total ?? 0} subtitle="Administrateurs + Pros + Etabs" icon={<Users className="w-4 h-4" />} accent={BRAND} />
                                    <KpiCard title="Professionnels" value={dataView.utilisateurs?.professionnels ?? 0} subtitle="Comptes Santé Pro" icon={<Users className="w-4 h-4" />} accent="#0041A8" />
                                    <KpiCard title="Établissements" value={dataView.utilisateurs?.etablissements ?? 0} subtitle="Centres médicaux / Écoles" icon={<ShoppingBag className="w-4 h-4" />} accent="#16A34A" />
                                    <KpiCard title="CA Total" value={`${(dataView.transactions?.montant_total ?? 0).toLocaleString()} CFA`} subtitle="Transactions réussies" icon={<TrendingUp className="w-4 h-4" />} accent="#10B981" />

                                    <KpiCard title="Pros à jour" value={dataView.professionnels?.ajour ?? 0} subtitle="Dossiers validés" icon={<CheckCircle2 className="w-4 h-4" />} accent="#16A34A" />
                                    <KpiCard title="Pros en attente" value={dataView.professionnels?.attente ?? 0} subtitle="Dossiers à traiter" icon={<Clock className="w-4 h-4" />} accent="#D97706" />
                                    <KpiCard title="Succès Paiements" value={dataView.transactions?.succes ?? 0} subtitle="Transactions OK" icon={<CheckCircle2 className="w-4 h-4" />} accent="#16A34A" />
                                    <KpiCard title="Échecs Paiements" value={dataView.transactions?.echec ?? 0} subtitle="Transactions KO" icon={<TrendingDown className="w-4 h-4" />} accent="#DC2626" />
                                </>
                            ) : (
                                <>
