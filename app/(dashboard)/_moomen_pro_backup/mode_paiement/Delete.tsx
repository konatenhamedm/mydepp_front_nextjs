...
export function Delete({ isOpen, onClose, onSuccess, data, size = "md" }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await apiFetch(`/modePaiements/${data.id}/delete`, { method: "DELETE" });
      toast.success("Mode de paiement supprimé !"); onSuccess(); onClose();
    ...
    >
      <div className="py-2">
        <p className="text-slate-600">
          Voulez-vous supprimer le mode de paiement <span className="font-bold text-red-600">{data?.libelle}</span> ?
        </p>
