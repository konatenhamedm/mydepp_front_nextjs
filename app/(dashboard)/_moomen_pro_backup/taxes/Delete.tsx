...
export function Delete({ isOpen, onClose, onSuccess, data, size = "md" }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await apiFetch(`/taxes/${data.id}/delete`, { method: "DELETE" });
      toast.success("Taxe supprimée !"); onSuccess(); onClose();
    ...
    >
      <div className="py-2">
        <p className="text-slate-600">
          Voulez-vous supprimer la taxe <span className="font-bold text-red-600">{data?.libelle}</span> ({data?.valeur}%) ?
        </p>
