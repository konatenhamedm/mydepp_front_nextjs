...
export function Delete({ isOpen, onClose, onSuccess, data, size = "md" }: Props) {
    ...
    const handleDelete = async () => {
        ...
        try {
            await apiFetch(`/civilite/delete/${data.id}`, {
                method: "DELETE"
            });
            toast.success("Civilité supprimée !");
            onSuccess();
            onClose();
        } catch (err: any) {
            ...
            <div className="py-4 text-center space-y-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                        Êtes-vous sûr de vouloir supprimer la civilité <span className="font-bold text-slate-700">"{data?.libelle}"</span> ?
