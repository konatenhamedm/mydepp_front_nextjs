...
export function Edite({ isOpen, onClose, onSuccess, data, size = "md" }: Props) {
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        try {
            await apiFetch(`/civilite/update/${data.id}`, {
                method: "PUT",
                data: form
            });
            toast.success("Civilité mise à jour !");
            onSuccess();
            onClose();
        } catch (err: any) {
        ...
        >
            <form onSubmit={handleSubmit} className="space-y-4 py-2">
                <div className="space-y-2">
                    ...
                    <Input value={form.code} onChange={set("code")} placeholder="Ex: M." required />
                </div>
                <div className="space-y-2">
                    <Label>Libellé <span className="text-red-500">*</span></Label>
                    <Input value={form.libelle} onChange={set("libelle")} placeholder="Ex: Monsieur" required />
                </div>
            </form>
        </Modal>
    );
