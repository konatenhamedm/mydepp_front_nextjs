...
export function Edite({ isOpen, onClose, onSuccess, data, size = "md" }: Props) {
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        try {
            await apiFetch(`/typeProfession/update/${data.id}`, {
                method: "PUT",
                data: form
            });
        ...
        <Modal
            ...
            title={
                <div className="flex items-center gap-3">
                    ...
                    Modifier le type de profession
                </div>
            }
            size={size}
            footer={
        ...
        >
            <form onSubmit={handleSubmit} className="space-y-4 py-2">
                <div className="space-y-2">
                    ...
                    <Input value={form.code} onChange={set("code")} placeholder="Ex: MED" required />
                </div>
                <div className="space-y-2">
                    <Label>Libellé <span className="text-red-500">*</span></Label>
                    <Input value={form.libelle} onChange={set("libelle")} placeholder="Ex: Médecin" required />
                </div>
            </form>
        </Modal>
    );
