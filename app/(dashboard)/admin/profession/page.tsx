    ...
    return (
        <ClientOnly>
            <div className="space-y-5">
                <PageHeader
                    title="Professions"
                    description="Gestion des professions et tarifs"
                ...
                <DataTable
                    ...
                    titleIcon={<Briefcase className="w-4 h-4" />}
                    footer={
                        <Pagination
                ...
                >
                    <table className="w-full">
                        <TableHeaderCustom
                            items={["Code", "Libellé", "Nouv. Demande", "Renouvellement", "Chrono Max"]}
                            afficheAction={true}
                            actionWidth="100px"
                        />
                        <tbody>
                            {isLoading ? (
                                ...
                                <EmptyState message="Aucune profession trouvée" icon={<Briefcase className="w-10 h-10" />} cols={COLS} />
                            ) : (
                                currentitems.map((item, index) => (
                                    <tr key={item.id ?? index} className="hover:bg-slate-50 transition-colors duration-100 group">
                                        ...
                                        <td className={TD.muted}>{item.montantNouvelleDemande} FCFA</td>
                                        <td className={TD.muted}>{item.montantRenouvellement} FCFA</td>
                                        <td className={TD.muted}>{item.chronoMax} jours</td>
