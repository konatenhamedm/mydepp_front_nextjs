...
import { Edit3, Phone, Mail, User, MapPin, MessageSquare } from "lucide-react";
...
export const Edite = ({ isOpen, onClose, data: client, onSuccess, size = "md" }: Props) => {
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    setIsLoading(true);
    try {
      await apiFetch("/clients/update", {
        method: "PUT",
        data: {
          id: client.id,
          ...data,
          magasin_id: magasinId || client.magasin_id,
        },
      });
    ...
    <Modal
      ...
      title={
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-white tracking-tight">Modifier le Client</h2>
            <p className="text-[10px] text-white/70 font-medium uppercase tracking-widest">Édition des informations</p>
          </div>
        </div>
      }
      size={size}
      footer={
        <ModalFooterButtons
          ...
          confirmText={isLoading ? "Modification..." : "Enregistrer les modifications"}
          isLoading={isLoading}
        />
      }
    >
