...
import {
    ...
    Calendar, Building2, Search, Filter, Download, FileText
} from "lucide-react";
...
export default function InstructeurDashboardPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const [statsRes, profsRes, jobsRes] = await Promise.all([
                apiFetch(statsUrl),
    ...
    const filteredItems = items.filter(item => {
        ...
        const professionId = item.personne?.profession?.id;
        ...
        const matchesType = filter === "all" ||
            ...
            (filter === "validated" && status === "valide") ||
            (filter === "a_jour" && status === "a_jour");

        // Profession Filter
        const matchesProfession = !selectedProfession || String(professionId) === selectedProfession;
        ...
        <ClientOnly>
            <div className="space-y-8 pb-12">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h1 className="text-3xl font-black text-slate-900 tracking-tight">Tableau de bord Instructeur</h1>
                        <p className="text-slate-500 font-light">Suivi et gestion des dossiers professionnels</p>
                    </div>
                    <div className="flex gap-3">
                        ...
                        <button className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-600/20">
                    ...
                    <div className="p-8 border-b border-slate-100 space-y-6">
                        <div className="flex flex-col md:flex-row justify-between gap-6">
                            <h2 className="text-xl font-black text-slate-900 flex items-center gap-3">
                                ...
                                Liste des dossiers
                                <Badge className="bg-blue-50 text-blue-600 border-blue-100">{filteredItems.length} résultats</Badge>
                            </h2>
                            ...
                            <div className="relative w-full md:w-96">
                                ...
                                <input
                                    ...
                                    className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-blue-500/10 transition-all font-medium text-sm"
                                />
                            </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-4">
                            ...
                            <select
                                ...
                                className="px-4 py-2 bg-white border border-slate-100 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                            >
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-slate-50/50 border-b border-slate-100">
                                    ...
                                    <th className="px-8 py-5 text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Professionnel</th>
                                    <th className="px-8 py-5 text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Contact</th>
                                    <th className="px-8 py-5 text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Profession</th>
                                    ...
                                    <tr>
                                        <td colSpan={6} className="px-8 py-20 text-center">
                                            <div className="flex flex-col items-center gap-4">
                                                ...
                                                <p className="text-slate-500 font-medium">Aucun dossier ne correspond à votre recherche</p>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                        ...
                                        <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                                            ...
                                            <td className="px-8 py-6">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center font-black text-sm uppercase">
                                                        {(item.personne?.nom?.[0] || "") + (item.personne?.prenoms?.[0] || "")}
                                                    </div>
                                                    <div>
                                            ...
                                            <td className="px-8 py-6">
                                                <div className="space-y-1">
                                                    ...
                                                    <p className="text-xs text-slate-400 truncate max-w-[180px]">{item.email}</p>
                                                </div>
                                            </td>
                                            <td className="px-8 py-6">
...
function StatCard({ label, value, icon, color, active, onClick }: { label: string, value: any, icon: React.ReactNode, color: string, active: boolean, onClick: () => void }) {
    const colors: any = {
        ...
        slate: "bg-slate-50 text-slate-500 border-slate-100 ring-slate-500"
    };
