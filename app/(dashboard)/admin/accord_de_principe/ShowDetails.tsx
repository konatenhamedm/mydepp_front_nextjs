"use client";
import React, { useState } from "react";
import { Modal } from "@/components/ui/Modal";
import { Info, Building2, MapPin, User, Mail, Phone, Calendar, ShieldCheck, FileText, Landmark, FileSignature, Receipt, Image as ImageIcon } from "lucide-react";
import { PrimaryButton } from "@/components/ui/page-components";
import { Badge } from "@/components/ui/badge";
import { Tabs, Tab } from "@nextui-org/react";
import { FicheInscription } from "@/app/(dashboard)/admin/components/business-docs/FicheInscription";
import { RecuPaiement } from "@/app/(dashboard)/admin/components/business-docs/RecuPaiement";
import { DocViewer } from "@/app/(dashboard)/admin/components/business-docs/DocViewer";
import { BASE_URL_UPLOAD } from "@/lib/axios";

interface Props { isOpen: boolean; onClose: () => void; data: any; size?: "sm" | "md" | "lg" | "xl" | "2xl" | "full"; }

export function ShowDetails({ isOpen, onClose, data, size = "full" }: Props) {
    const [selectedDoc, setSelectedDoc] = useState<string | null>(null);

    if (!data) return null;
    const p = data.personne || {};

    const getDocUrl = (url: string) => {
        if (!url) return null;
        if (url.startsWith('http')) return url;
        return `${BASE_URL_UPLOAD}${url}`;
    };

    return (
        <Modal
            isOpen={isOpen}
            onClose={() => { setSelectedDoc(null); onClose(); }}
            title={
                <div className="flex items-center gap-3">
                    <div className="bg-[#EBF2FF] p-2 rounded-lg text-[#0052CC]"><FileSignature className="h-5 w-5" /></div>
                    Détails Accord de Principe
                </div>
            }
            size={size}
            footer={
                <div className="flex justify-end w-full">
                    <PrimaryButton onClick={onClose} className="px-6">Fermer</PrimaryButton>
                </div>
            }
        >
            <div className="py-2 space-y-6">
                {/* Header Section */}
                <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div className="flex items-center gap-4">
                        <div className="w-16 h-16 bg-white border border-slate-200 rounded-full flex items-center justify-center text-[#0052CC] shadow-sm">
                            <Building2 className="w-8 h-8" />
                        </div>
                        <div>
                            <h2 className="text-2xl font-bold text-slate-900 tracking-tight">
                                {p.denomination || p.nomRepresentant || 'Établissement ACP'}
                            </h2>
                            <div className="flex items-center gap-2 mt-1 text-slate-500 font-medium text-sm">
                                <Landmark className="w-4 h-4" /> {p.typeEtablissement?.libelle || 'Dossier Accord de Principe'}
                            </div>
                        </div>
                    </div>
                    <Badge className="bg-[#0052CC] text-white border-0 shadow-lg px-4 py-1.5 flex items-center gap-2">
                        <ShieldCheck className="w-4 h-4" /> {p.status?.replace(/_/g, ' ')}
                    </Badge>
                </div>

                <Tabs
                    aria-label="Options de détails"
                    color="primary"
                    variant="underlined"
                    classNames={{
                        tabList: "gap-6 w-full relative rounded-none p-0 border-b border-divider",
                        cursor: "w-full bg-[#0052CC]",
                        tab: "max-w-fit px-0 h-12",
                        tabContent: "group-data-[selected=true]:text-[#0052CC] font-semibold text-slate-500"
                    }}
                    onSelectionChange={() => setSelectedDoc(null)}
                >
                    <Tab key="informations" title="Informations">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4 px-2">
                            <div className="space-y-6">
                                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2 border-b border-slate-100 pb-2">
                                    <Info className="w-4 h-4" /> Identification
                                </h3>
                                <div className="space-y-4">
                                    <InfoRow icon={<User className="w-4 h-4" />} label="Représentant" value={p.nomRepresentant || '—'} />
                                    <InfoRow icon={<Mail className="w-4 h-4" />} label="Email" value={p.emailAutre || data.email || '—'} />
                                    <InfoRow icon={<Phone className="w-4 h-4" />} label="Téléphone" value={p.telephone || '—'} />
                                    <InfoRow icon={<FileText className="w-4 h-4" />} label="Type Juridique" value={p.typePersonne?.libelle || '—'} />
                                </div>
                            </div>

                            <div className="space-y-6">
                                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2 border-b border-slate-100 pb-2">
                                    <MapPin className="w-4 h-4" /> Localisation & Suivi
                                </h3>
                                <div className="space-y-4">
                                    <InfoRow icon={<MapPin className="w-4 h-4" />} label="Localisation" value={`${p.commune?.libelle || ''}, ${p.ville?.libelle || ''}`} />
                                    <InfoRow icon={<Calendar className="w-4 h-4" />} label="Mise à jour" value={data.updatedAt ? new Date(data.updatedAt).toLocaleDateString() : '—'} />
                                    <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100 mt-4">
                                        <span className="text-[10px] font-bold text-amber-600 uppercase tracking-wider block mb-1">Instructeur Assigné</span>
                                        {p.imputationData ? (
                                            <div className="flex items-center gap-2 font-bold text-amber-800">
                                                <div className="w-6 h-6 rounded-full bg-amber-200 flex items-center justify-center text-[10px]"><User className="w-3 h-3" /></div>
                                                {p.imputationData.username}
                                            </div>
                                        ) : (
                                            <span className="text-amber-400 text-xs italic">Non assigné</span>
                                        )}
                                    </div>
                                </div>
                            </div>
                        </div>
                    </Tab>

                    <Tab key="documents" title="Documents & Étude">
                        <div className="flex flex-col md:flex-row gap-6 mt-4">
                            <div className="w-full md:w-1/3 flex flex-col gap-2">
                                <h3 className="text-sm font-bold text-slate-800 mb-2 px-1">Pièces du dossier</h3>

                                <DocButton title="Fiche d'Inscription" icon={<FileSignature className="w-4 h-4" />} isActive={selectedDoc === "fiche"} onClick={() => setSelectedDoc("fiche")} variant="blue" />
                                <DocButton title="Reçu de Paiement" icon={<Receipt className="w-4 h-4" />} isActive={selectedDoc === "recu"} onClick={() => setSelectedDoc("recu")} variant="green" />

                                <div className="h-px bg-slate-200 my-2" />

                                {p.documents?.map((doc: any, idx: number) => (
                                    <DocButton
                                        key={idx}
                                        title={doc.libelle || `Document ${idx + 1}`}
                                        icon={doc.path?.url?.toLowerCase().includes('.pdf') ? <FileText className="w-4 h-4" /> : <ImageIcon className="w-4 h-4" />}
                                        isActive={selectedDoc === `doc-${idx}`}
                                        onClick={() => setSelectedDoc(`doc-${idx}`)}
                                        disabled={!doc.path?.url}
                                    />
                                ))}

                                {(!p.documents || p.documents.length === 0) && (
                                    <p className="text-xs text-slate-400 italic p-3 bg-slate-50 rounded-lg">Aucune pièce jointe.</p>
                                )}
                            </div>

                            <div className="w-full md:w-2/3 min-h-[500px] border border-slate-200 rounded-xl bg-slate-50 overflow-hidden flex flex-col">
                                {!selectedDoc ? (
                                    <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-8 text-center">
                                        <FileText className="w-12 h-12 mb-3 opacity-20" />
                                        <p className="font-medium text-slate-500">Choisissez un document pour l'examiner</p>
                                    </div>
                                ) : (
                                    <>
                                        {selectedDoc === "fiche" && <FicheInscription data={data} />}
                                        {selectedDoc === "recu" && <RecuPaiement data={data} />}
                                        {selectedDoc.startsWith("doc-") && (
                                            <DocViewer
                                                url={getDocUrl(p.documents[parseInt(selectedDoc.split('-')[1])].path?.url)}
                                                alt={p.documents[parseInt(selectedDoc.split('-')[1])].libelle}
                                            />
                                        )}
                                    </>
                                )}
                            </div>
                        </div>
                    </Tab>
                </Tabs>
            </div>
        </Modal>
    );
}

function InfoRow({ icon, label, value }: { icon: React.ReactNode, label: string, value: string }) {
    if (!value || value.trim() === ',' || value === '—') {
        value = 'Non renseigné';
    }
    return (
        <div className="flex items-start gap-4">
            <div className="mt-1 text-slate-300">{icon}</div>
            <div>
                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">{label}</span>
                <span className="text-sm font-semibold text-slate-700">{value}</span>
            </div>
        </div>
    );
}

function DocButton({ title, icon, isActive, onClick, disabled, variant = "default" }: any) {
    const baseClass = "flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all border text-left w-full";
    let variantClass = "border-slate-200 bg-white text-slate-700 hover:bg-slate-50 hover:border-slate-300";

    if (disabled) {
        variantClass = "border-slate-100 bg-slate-50 text-slate-400 cursor-not-allowed opacity-60";
    } else if (isActive) {
        if (variant === "blue") variantClass = "border-[#0052CC] bg-blue-50 text-[#0052CC] shadow-[0_4px_12px_rgba(0,82,204,0.1)]";
        else if (variant === "green") variantClass = "border-emerald-600 bg-emerald-50 text-emerald-700 shadow-[0_4px_12px_rgba(5,150,105,0.1)]";
        else variantClass = "border-slate-800 bg-slate-800 text-white shadow-md";
    }

    return (
        <button onClick={onClick} disabled={disabled} className={`${baseClass} ${variantClass}`}>
            {icon}
            <span className="flex-1 truncate uppercase text-[10px] tracking-wider">{title}</span>
        </button>
    );
}
