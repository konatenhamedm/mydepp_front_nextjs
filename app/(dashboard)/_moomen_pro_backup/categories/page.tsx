...
import { Plus, Tag, Store } from "lucide-react";
...
export default function CategoriesPage() {
    ...
    const refreshData = () => {
        if (!magasinId) return;
        setIsLoading(true);
        apiFetch(`/categorie_produit_services/all/magasin/${magasinId}`)
    ...
    return (
        <ClientOnly>
            <div className="space-y-5">
                <PageHeader
                    title="Catégories"
                    description={`Catégories du magasin ${magasin?.libelle ?? ""}`}
                ...
                >
                    <table className="w-full">
                        <TableHeaderCustom items={["#", "Libellé"]} afficheAction={true} actionWidth="100px" />
                        <tbody>
                            {isLoading ? (
                                <TableSkeletonRows cols={COLS} />
                            ) : currentitems.length === 0 ? (
                                <EmptyState message="Aucune catégorie trouvée" icon={<Tag className="w-10 h-10" />} cols={COLS} />
                            ) : (
