...
export default function CommunePage() {
    ...
    const filteredData = Array.isArray(data)
        ? data.filter((item) =>
            ...
            item.ville_libelle?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            item.ville?.libelle?.toLowerCase().includes(searchTerm.toLowerCase())
        )
        : [];
    ...
    return (
        <ClientOnly>
            <div className="space-y-5">
                <PageHeader
                    title="Communes"
                    description="Gestion des communes"
                ...
                <SearchBar
                    ...
                    placeholder="Rechercher par libellé ou ville..."
                    onRefresh={refreshData}
                    isLoading={isLoading}
                />
                <DataTable
                    title="Liste des communes"
                    titleIcon={<Home className="w-4 h-4" />}
                    footer={
                        <Pagination
                    ...
                    <table className="w-full">
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                <EmptyState message="Aucune commune trouvée" icon={<Home className="w-10 h-10" />} cols={COLS} />
                            ) : (
                                currentitems.map((item, index) => (
                                    <tr key={item.id ?? index} className="hover:bg-slate-50 transition-colors duration-100 group">
                                        ...
                                        <td className={TD.muted}>{item.ville_libelle || item.ville?.libelle || "N/A"}</td>
