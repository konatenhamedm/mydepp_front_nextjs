...
export default function GroupeLibellePage() {
    ...
    const getTypeLibelle = (type: string) => {
        switch (type) {
            case "ACP": return "Accord de principe";
            case "OEP": return "Ouverture d'Exploitation";
    ...
    return (
        <ClientOnly>
            <div className="space-y-5">
                <PageHeader
                    title="Groupes de Libellés"
                    description="Gestion des groupes de libellés pour les types de dossiers"
                    ...
                    <table className="w-full">
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                <EmptyState message="Aucun groupe trouvé" icon={<Tags className="w-10 h-10" />} cols={COLS} />
                            ) : (
                                currentitems.map((item, index) => (
                                    <tr key={item.id ?? index} className="hover:bg-slate-50 transition-colors duration-100 group">
                                        ...
                                        <td className={TD.bold}>{getTypeLibelle(item.type)}</td>
