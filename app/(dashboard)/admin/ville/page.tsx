...
export default function VillePage() {
    ...
    const filteredData = Array.isArray(data)
        ? data.filter((item) =>
            ...
            item.district_libelle?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            item.district?.libelle?.toLowerCase().includes(searchTerm.toLowerCase())
        )
        : [];
        ...
        <ClientOnly>
            <div className="space-y-5">
                ...
                <SearchBar
                    ...
                    placeholder="Rechercher par libellé, code ou district..."
                    ...
                    <table className="w-full">
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                <EmptyState message="Aucune ville trouvée" icon={<MapPin className="w-10 h-10" />} cols={COLS} />
                            ) : (
                                currentitems.map((item, index) => (
                                    <tr key={item.id ?? index} className="hover:bg-slate-50 transition-colors duration-100 group">
                                        ...
                                        <td className={TD.muted}>{item.district_libelle || item.district?.libelle || "N/A"}</td>
