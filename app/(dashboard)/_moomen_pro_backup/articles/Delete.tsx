...
export function Delete({ isOpen, onClose, onSuccess, data, size = "md" }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await apiFetch(`/articles/${data.id}/delete`, { method: "DELETE" });
      toast.success("Article supprimé !"); onSuccess(); onClose();
    ...
    >
      <div className="py-2">
        <p className="text-slate-600">
          Voulez-vous supprimer l'article <span className="font-bold text-red-600">{data?.libelle ?? data?.id}</span> ?
        </p>
