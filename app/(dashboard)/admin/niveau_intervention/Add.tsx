...
export function Add({ isOpen, onClose, onSuccess, size = "md" }: Props) {
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        try {
            await apiFetch("/niveauIntervention/create", {
                method: "POST",
                data: form
            });
            toast.success("Niveau crée !");
            setForm({ libelle: "" });
            onSuccess();
            onClose();
        } catch (err: any) {
        ...
        <Modal
            ...
            title={
                <div className="flex items-center gap-3">
                    <div className="bg-[#EBF2FF] p-2 rounded-lg text-[#0052CC]"><Layers className="h-5 w-5" /></div>
        ...
        >
            <form onSubmit={handleSubmit} className="space-y-4 py-2">
                <div className="space-y-2">
                    ...
                    <Input value={form.libelle} onChange={set("libelle")} placeholder="Ex: National" required />
                </div>
            </form>
        </Modal>
    );
